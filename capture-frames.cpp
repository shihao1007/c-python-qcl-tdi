...
void CreateDisplayImageExample(HANDLE_IPS_ACQ handle_ips, int fpg, std::string dest_sub_path)
{
	...
	{
		// Get a pointer to the image
		uint16_t * p_single_frame = (uint16_t* ) (display_image.data() + frame_index*frame_data_size);

		const std::string TXTfilename = GetPGMFileName(dest_sub_path, "sbf161_img_*", frame_index, 1600);
		std::stringstream grabindex;
		grabindex << "s";
		stim::save_mat4((char*)p_single_frame, TXTfilename,grabindex.str(), 128, 128, stim::mat4_int32);
...
void comp_imaging(int wn, int QCL_index, int QCL_MaxCur, HANDLE_IPS_ACQ handle,
				   int frames, std::string dest_sub_path, uint32_t threshold, int laserpower_low){
					  ...
					  while (!santurate){
						  ...
						  uint32_t mean = CalculateMean(handle, frames);
						  tuning_count++;
...
int main(int argc, char* argv[]) {
	...
	args.add("frames", "total number of images to collect", "300", "integer (currently between 1 and 500)");
	args.add("WN", "wavenumber to do imaging at", "1406", "integer (currently between 910 and 1900)");
	...
	//
		...
		std::cout << "Tuning to WN :" << wn << std::endl;
			...
			comp_imaging(wn, 3, 1000, hcam, frames, dest_sub_path, 9300, 60);
