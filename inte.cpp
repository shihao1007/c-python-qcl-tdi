...
void SaveGrayScalePGM(uint16_t * p_pixel_data, int width, int height, int frame_number, const std::string pgm_path) 
{ 
  std::ofstream ofile(pgm_path, std::ios::out|std::ios::ate);
  //ofile << "P2\n" << width << " " << height << "\n16383\n";
  int num_pixels = width*height*frame_number;
...
void holo_capture(int wn, int positions, int QCL_index, int QCL_MaxCur, HANDLE_IPS_ACQ handle, DOUBLE result_stage,
				  std::string cmd_step, std::string cmd_return, int frames, int lp_low){

				uint32_t ret;												//return value used by MIRcat laser control
				bool IsOn = false;
				int lp_high = 100;
				int pnum = lp_high - lp_low;
				...
				for (int cnum = 312; cnum < 320; cnum = cnum + 8){

					std::stringstream sub_dir;												//create an empty string stream
					sub_dir << dest_path << cnum << "\\";												//append to the parent dir string
					...
					int R_tick = cnum / 140 + 1;
					int C_tick = cnum % 140;
						  
					int32_t intewrite = IntegrationTimeWriteExample(handle, R_tick, C_tick);
					double integration_time_readout(0);
					int32_t result = IPS_GetDoubleParam(handle,
										IPS_READ_INTEGRATION_TIME,
										&integration_time_readout);

					std::cout << std::setprecision(8) << std::fixed;
					std::cout<<"Row = "<< R_tick<<"; "<<"Col = "<<C_tick<<";"<< std::endl;
					std::cout<<"integration time ="<< integration_time_readout<< std::endl;


					for (int lp = lp_low; lp < lp_high; lp = lp + 2 ){
						...
						//perform an imaging pass across the sample
							...
							CreateDisplayImageExample(handle, lp, frames, dest_sub_path);											//capture images		 
							//					  std::chrono::high_resolution_clock::time_point t3 = std::chrono::high_resolution_clock::now();
...
int main(int argc, char* argv[]) {
	...
	args.add("laserpower","laser power to do imaging at","98","integer (currently between 1 and 100)");
